...
class KafkaMonitor(object):
    ...
    def _load_plugins(self):
        ...
        self.plugins_dict = OrderedDict(sorted(list(self.plugins_dict.items()), key=lambda t: t[0]))
    ...
    def _setup_stats_plugins(self, redis_conn):
        ...
        for key in self.plugins_dict:
            ...
            total = StatsCollector.get_hll_counter(redis_conn=redis_conn,
                                                   ...
                                                   cycle_time=self.settings['STATS_CYCLE'],
                                                   roll=False)
            self.logger.debug("Set up {p} plugin Stats Collector 'lifetime'".format(p=plugin_name))
            ...
            for message in self.consumer:
                ...
                try:
                    ...
                    for key in self.plugins_dict:
                        ...
                        schema = obj['schema']
                        try:
                            self.validator(schema).validate(the_dict)  # 数据校验
                            found_plugin = True
